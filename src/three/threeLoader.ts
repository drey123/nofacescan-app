...
export function loadScene() {
  ...
  // Keep the original GLB on its own object root. Reconstruction never edits this tree.
...
export async function setImageFace(
  ...
  try {
    const controller = await createFaceVerseMesh(image, bbox, onProgress, signal);
    ...
    reconstructedFace.visible = true;
    scene.add(reconstructedFace);
    updateModel();
    return true;
  } catch (error) {
    // Never leave the original GLB hidden after a failed/cancelled reconstruction.
...
function clearReconstructedFace() {
  ...
  if (Array.isArray(reconstructedFace.material)) {
    reconstructedFace.material.forEach(material => material.dispose());
  } else {
    reconstructedFace.material.dispose();
  }
...
export function updateModel(doRender = true) {
  ...
  // Existing GLB controls remain unchanged.
  if (faceMesh) {
  ...
  // The same UI controls drive the reconstructed face. This keeps the control contract shared.
